...
submitWordBtn.addEventListener('click', () => {
  ...
  if (word.length === wordLength) {
    ...
    gameStatus.textContent = `Число должно содержать ${wordLength} цифр/ы`;
  }
});
...
submitGuessBtn.addEventListener('click', () => {
  ...
  if (guess.length === wordLength) {
    ...
    gameStatus.textContent = `Догадка должна содержать ${wordLength} цифр/ы`;
  }
});
...
document.addEventListener("DOMContentLoaded", () => {
	...
	if (guessInput) allowOnlyNumbers(guessInput);
	if (secretWord) allowOnlyNumbers(secretWord);
});

function updateInputFields(length) {
  // Обновляем атрибуты полей ввода
  secretWordInput.maxLength = length;
  secretWordInput.pattern = `[0-9]{${length}}`;
  secretWordInput.placeholder = `Введите число из ${length} цифр`;
  secretWordInput.title = `Число должно содержать ${length} цифр/ы`;
  
  guessInput.maxLength = length;
  guessInput.pattern = `[0-9]{${length}}`;
  guessInput.placeholder = `Введите догадку из ${length} цифр`;
  guessInput.title = `Догадка должна содержать ${length} цифр/ы`;
...
socket.on('wordly_room_joined', (data) => {
  roomId = data.roomId;
  playerId = socket.id;
  wordLength = data.wordLength; // Получаем длину числа из данных комнаты
  updateInputFields(wordLength);
  
  lobby.classList.add('hidden');
  game.classList.remove('hidden');
  gameStatus.textContent = `Вы присоединились к комнате. Введите число из ${wordLength} цифр`;
  leaveLobbyBtn.classList.remove('hidden');
  document.getElementById('roomCode').textContent = roomId;
  
  // Обновляем подсказки в полях ввода
  secretWordInput.placeholder = `Введите число из ${wordLength} цифр`;
  secretWordInput.pattern = `[0-9]{${wordLength}}`;
  guessInput.placeholder = `Введите догадку из ${wordLength} цифр`;
  guessInput.pattern = `[0-9]{${wordLength}}`;
});
