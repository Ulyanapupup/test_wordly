document.addEventListener('DOMContentLoaded', () => {
    ...
    createRoomBtn.addEventListener('click', () => {
        socket.emit('wordly_create_room');
    });
    
    joinRoomBtn.addEventListener('click', () => {
        const id = roomIdInput.value.trim();
        if (id) {
            socket.emit('wordly_join_room', { room_id: id });
        }
    });
    ...
    submitWordBtn.addEventListener('click', () => {
        ...
        if (word.length === 5) {
            socket.emit('wordly_submit_word', { room_id: roomId, word });
    ...
    submitGuessBtn.addEventListener('click', () => {
        ...
        if (guess.length === 5) {
            socket.emit('wordly_make_guess', { room_id: roomId, guess });
    ...
    socket.on('wordly_update_words', (data) => {
        if (data.words[playerId]) {
            myWordDisplay.textContent = data.words[playerId];
        }
        const opponentId = Object.keys(data.words).find(id => id !== playerId);
        if (opponentId && data.words[opponentId]) {
            opponentWordDisplay.textContent = data.words[opponentId];
        }
    });
});