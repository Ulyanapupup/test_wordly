...
def is_greater(x, secret_number): 
    # Исправлено сравнение для отрицательных чисел
...
def is_prime(x, secret_number):
    ...
    for i in range(2, int(abs(secret_number) ** 0.5) + 1):
        if secret_number % i == 0: return False
    return True

def is_positive(x, secret_number): return secret_number > 0
def is_negative(x, secret_number): return secret_number < 0
...
def process_question(q):
    ...
    # Обработка вопроса о сумме цифр
    if "сумма цифр" in q:
